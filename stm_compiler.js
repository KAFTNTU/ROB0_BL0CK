...
const OP = {
    ...
    /* --- Маніпулятор / PCA9685 Servo 0x0A-0x0D --- */
    SERVO_SET:   0x0A,  /* [channel 1..16, angle 0..180] */
    SERVO4_SET:  0x0B,  /* [base, shoulder, elbow, grip] angles */
    SERVO_GRIP:  0x0C,  /* [mode 0=open,1=close,2=half] */
    SERVO_OFF:   0x0D,  /* [channel 1..16] full-off PWM */
...
class Compiler {
    ...
    compileStmt(block) {
        ...
        switch (block.type) {
            ...
            /* ---- Маніпулятор PCA9685 / Servo ---- */
            case 'manip_servo_named': {
                const map = { BASE:1, SHOULDER:2, ELBOW:3, GRIP:4 };
                const ch = map[block.getFieldValue('SERVO') || 'BASE'] || 1;
                const angle = this.staticNum(block.getInputTargetBlock('ANGLE'), 90);
                this.emit(OP.SERVO_SET); this.emit(ch); this.emit(Math.max(0, Math.min(180, Math.round(angle))));
                break;
            }
            case 'manip_servo_channel': {
                const ch = parseInt(block.getFieldValue('CH') || '1');
                const angle = this.staticNum(block.getInputTargetBlock('ANGLE'), 90);
                this.emit(OP.SERVO_SET); this.emit(Math.max(1, Math.min(16, ch))); this.emit(Math.max(0, Math.min(180, Math.round(angle))));
                break;
            }
            case 'manip_set4': {
                this.emit(OP.SERVO4_SET);
                ['BASE','SHOULDER','ELBOW','GRIP'].forEach(name => {
                    const a = this.staticNum(block.getInputTargetBlock(name), name === 'GRIP' ? 30 : 90);
                    this.emit(Math.max(0, Math.min(180, Math.round(a))));
                });
                break;
            }
            case 'manip_grip': {
                const mode = block.getFieldValue('MODE') || 'OPEN';
                const m = mode === 'CLOSE' ? 1 : (mode === 'HALF' ? 2 : 0);
                this.emit(OP.SERVO_GRIP); this.emit(m);
                break;
            }
            case 'manip_pose': {
                const poses = {
                    HOME:      [90,90,90,30],
                    TRANSPORT: [90,70,120,30],
                    PICK:      [90,125,55,85],
                    UP:        [90,55,70,30]
                };
                const p = poses[block.getFieldValue('POSE') || 'HOME'] || poses.HOME;
                this.emit(OP.SERVO4_SET);
                for (const a of p) this.emit(a);
                break;
            }
            case 'manip_servo_smooth': {
                /* У FRAM-байткоді без окремого VM-циклу: зберігаємо кінцевий кут.
                   Плавність можна додати у прошивці як розширення OP_SERVO_SET або окремий OP. */
                const map = { BASE:1, SHOULDER:2, ELBOW:3, GRIP:4 };
                const ch = map[block.getFieldValue('SERVO') || 'BASE'] || 1;
                const angle = this.staticNum(block.getInputTargetBlock('ANGLE'), 90);
                this.emit(OP.SERVO_SET); this.emit(ch); this.emit(Math.max(0, Math.min(180, Math.round(angle))));
            ...
            case 'manip_servo_off': {
                const ch = parseInt(block.getFieldValue('CH') || '1');
                this.emit(OP.SERVO_OFF); this.emit(Math.max(1, Math.min(16, ch)));
                break;
            }
    ...
    staticNum(block, def) {
        if (!block) return def;
        if (block.type === 'math_number' || block.type === 'math_number_limited' || block.type === 'servo_angle_number')
...
window.uploadToRobot = async function () {
    ...
    try {
        ...
        /* RB_SERVO_AUTO_RUN_AFTER_UPLOAD:
           після завантаження одразу запускаємо програму, щоб Scratch-блок серво
           не вимагав натискати OK на платі. */
        await sendPkt([PCMD.RUN]);
        _log('  → CMD_RUN: програму запущено', 'tx');
        await new Promise(r => setTimeout(r, 60));
